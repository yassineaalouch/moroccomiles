...
export function CityCard({ destination, index }: { destination: Destination; index: number }) {
  ...
  return (
    <motion.button
      ...
      >
        <Image src={destination.hero} alt={`${destination.name}, Morocco`} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-all duration-300 group-hover:scale-105" />
        ...
        <div className="absolute inset-x-0 bottom-0 p-7 sm:p-9">
          ...
          <p className="mt-3 max-w-sm text-sm leading-6 text-morocco-sand/75">{destination.tagline}</p>
        </div>
    </motion.button>
  );
}
